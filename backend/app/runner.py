...
#: Demucs renders a tqdm bar; this catches its percentage on either stream.
...
def build_argv(
    ...
    argv += ["-o", str(output_dir), "--filename", options.filename]
    ...
    if probe.choice_supported("--clip-mode", options.clip_mode):
    ...
    try:
        ...
        for chunk in _iter_lines(process.stdout):
            ...
            if on_progress and (now - last_emit > 0.5 or match):
                last_emit = now
                on_progress(tracker.snapshot("separating"))
            ...
            if deadline and now > deadline:
                _terminate(process)
                raise TimeoutError(
                    f"Demucs superó el tiempo máximo de {timeout}s y fue detenido."
                )
